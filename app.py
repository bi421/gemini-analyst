...
<meta name="apple-mobile-web-app-title" content="Groq AI">
...
<style>
  ...
  /* Spinner */
  .stSpinner { font-size: 15px !important; }
</style>
...
def read_youtube(url):
    ...
    if gemini_model:
        try:
            ...
            response = gemini_model.generate_content([img, "Энэ YouTube thumbnail зургийг Монгол хэлээр дэлгэрэнгүй тайлбарла."])
...
def edit_image_with_gemini(image, instruction):
    ...
    try:
        response = gemini_model.generate_content([image, f"Энэ зургийг дараах байдлаар засварлаж тайлбарла: {instruction}"])
